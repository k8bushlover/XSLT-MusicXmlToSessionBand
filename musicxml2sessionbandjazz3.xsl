...
<!-- Author: Kent Finley -->
<!-- Purpose: Convert MusicXML file as exported by iReal Pro to JSON format required by SessionBand Jazz Volume 3 -->
...
<!-- e.g. 
     <?xml-stylesheet type="text/xsl" href="file://C:\Users\username\Desktop\musicxml2sessionbandjazz3.xsl"?>
		can omit the path and just give the filename, if files in same directory
-->	
<!-- then drag the .xml file to the current address bar in IE, save resulting text (and text ONLY) in a text file with .sbj3 extension -->
<!-- import .sbj3 file into SessionBand Jazz Volume 3 via iTunes -->
...
<!-- not extracting key signature from MusicXML file, as it doesn't really matter in SessionBand Jazz Volume 3 -->
...
-->
    ...
    <template match="score-partwise">
		<text>{"Genre":</text>
		<choose>
			<when test="($beats mod 3 = 0) and ($beats mod 2 != 0)">"Slow 3/4",</when>
			<when test="($beats mod 2 = 0)">"Trad Jazz",</when>
			<when test="($beats = 7)">"7/4 Even 8ths",</when>
			<otherwise>"Jazz Ballad",</otherwise>
		</choose>
		<!-- 'Jazz Ballad' is probably NOT appropriate for time signatures not matching one of the three cases above -->
		<!-- but at least it will result in a valid SBJ3 format -->
		<!-- COMPLETE list for SessionBand Jazz Vol. 3
			Jazz Ballad
			Swing Funk
			Slow ECM
			Slow 3/4
    ...
    <template match="kind">
		...
		<choose>
			...
			<!-- types in SessionBand Jazz Volume 3: 
					0=maj7(9)
					1=maj7#11  - altered
					2=7(13)
					3=7(b9) - altered
